...
final class DateTimeParser  {
  ...
  */
  ...
  private Parts splitIntoDateAndTime(String aDateTime){
    ...
    int space = aDateTime.indexOf(SPACE);
    boolean hasSpaceInMiddle = 0 < space  && space < aDateTime.length();
    if (hasSpaceInMiddle){
      result.datePart = aDateTime.substring(0, space);
      result.timePart = aDateTime.substring(space+1);
    }
